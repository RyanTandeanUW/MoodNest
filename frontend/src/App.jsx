...
function App() {
  ...
  // Audio Logic - Background music based on mood
      ...
      try {
        ...
        if (data?.vibe_details?.track) {
          ...
          if (isMuted) {
            ...
            audioRef.current
              .play()
              .then(() => console.log("🎵 Music playing:", data.vibe_name))
              .catch(() =>
                console.log("⏸️ Audio waiting for user interaction..."),
              );
          }
        } else {
          // If no track (like in neutral), stop and clear the music completely
          audioRef.current.pause();
          audioRef.current.src = ""; // Clear source to fully stop playback
          audioRef.current.currentTime = 0; // Reset to beginning
          console.log(
            "⏸️ No track for neutral mood - music stopped and cleared",
          );
        }
      } catch (e) {
  ...
  const handleCollapse = async () => {
    ...
    try {
      await fetch("http://localhost:8000/action/reset", { method: "POST" });
  ...
  // Mood-based background gradient colors
  const moodGradients = {
    happy: {
      blob1: "from-yellow-500 via-amber-500 to-orange-500",
      blob2: "from-orange-600 via-yellow-500 to-amber-600",
    },
    sad: {
      blob1: "from-blue-600 via-indigo-600 to-cyan-600",
      blob2: "from-cyan-700 via-blue-600 to-indigo-700",
    },
    angry: {
      blob1: "from-red-600 via-rose-600 to-pink-600",
      blob2: "from-pink-700 via-red-600 to-rose-700",
    },
    neutral: {
      blob1: "from-indigo-600 via-purple-600 to-pink-600",
      blob2: "from-purple-700 via-pink-600 to-indigo-700",
    },
  };

  const currentGradients = moodGradients[currentMood] || moodGradients.neutral;

  return (
    <div className="min-h-screen relative overflow-hidden bg-slate-900">
      {/* Background waves - change color based on mood */}
      <div className="absolute inset-0 wave opacity-40">
        <div
          className={`absolute top-1/4 left-1/4 w-[800px] h-[800px] rounded-full bg-gradient-to-br ${currentGradients.blob1} blur-3xl transition-all duration-1000`}
        ></div>
      </div>
      <div className="absolute inset-0 wave-slow opacity-30">
        <div
          className={`absolute top-1/3 right-1/4 w-[600px] h-[600px] rounded-full bg-gradient-to-tl ${currentGradients.blob2} blur-3xl transition-all duration-1000`}
        ></div>
      </div>
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-8">
        ...
        >
          <div
            className={`transition-opacity duration-700 ${isExpanded || isTransitioning ? "opacity-0" : "opacity-100"}`}
          >
            {!isExpanded && <Hero />}
          </div>
